...
import com.example.collisioncalc.data.lookups.FakeVinDecoder
import com.example.collisioncalc.data.lookups.VinDecoder
...
fun VehicleDetailScreen(
    ...
    val vinDecoder: VinDecoder = remember { FakeVinDecoder() }
                                ...
                                onClick = {
                                    ...
                                    scope.launch {
                                        ...
                                        try {
                                            ...
                                            if (decoded == null) {
                                                vinDecodeStatus = "No decode data."
                                            } else {
                                                var applied = false
                                                ...
                                                vinDecodeStatus =
                                                    if (applied) "Applied decoded fields."
                                                    else "Nothing to fill (fields already set or decode empty)."
                                            }
                                        } catch (_: Exception) {
                                            vinDecodeStatus = "Decode failed."
                                        } finally {
                                            isDecodingVin = false
                                        }
                                    }
                            ...
                            onClick = {
                                val calc = SavedCalculation(
                                    ...
                                    inputs = listOf(
                                        ...
                                        CalcValue("Indicated (${knownSpeed.label})", abs(knownSpeed.mph), "mph"),
                                    ...
                                    steps = buildList {
                                        ...
                                        add("Indicated (${knownSpeed.label}) = ${fmt(knownSpeed.mph, 4)} mph")
                                        add("Stock diameter = ${fmt(stockDiaIn, 6)} in")
                                        add("Current diameter = ${fmt(curDiaIn, 6)} in")
                                        add("Multiplier = D_current / D_stock = ${fmt(ratio, 6)}")
                                        add("Corrected = ${fmt(knownSpeed.mph, 6)} × ${fmt(ratio, 6)} = ${fmt(correctedSpeed, 6)} mph")
                                    },
