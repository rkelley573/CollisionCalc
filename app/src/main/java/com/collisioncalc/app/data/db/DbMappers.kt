...
import com.collisioncalc.app.data.CaseNote
...
import com.collisioncalc.app.data.NameParts
...
import com.collisioncalc.app.data.UnitKind
    ...
    fun caseToRows(caseFile: CaseFile): CaseSnapshotRows {
        ...
        val valueRows = buildList {
            caseFile.calculations.forEach { c ->
                c.inputs.forEach { v ->
                    add(CalcValueRow(calcId = c.calcId, role = "IN", name = v.name, value = v.value, unit = v.unit, vehicleId = v.vehicleId))
                }
                c.outputs.forEach { v ->
                    add(CalcValueRow(calcId = c.calcId, role = "OUT", name = v.name, value = v.value, unit = v.unit, vehicleId = v.vehicleId))
                }
            }
        }
    ...
    suspend fun rowsToCase(
        ...
        val domainVehicles = vehicles.map { v ->
            val occs = occByVehicle[v.vehicleId].orEmpty()
                ...
                .map { o ->
                    Occupant(
                        name = NameParts(last = o.last, first = o.first, middle = o.middle, suffix = o.suffix),
                        dobIso = o.dobIso,
                        seatingPosition = o.seatingPosition,
            ...
            Vehicle(
                ...
                weightLb = v.weightLb,
                occupants = occs,
                insurance = InsuranceInfo(company = v.insCompany, policyNumber = v.insPolicyNumber, phone = v.insPhone),
                stockTireWidthMm = v.stockTireWidthMm,
                stockTireAspectPct = v.stockTireAspectPct,
                stockTireWheelIn = v.stockTireWheelIn,
                currentTireWidthMm = v.currentTireWidthMm,
        ...
        val domainUnits: List<UnitEntity> = units.map { u ->
            when (u.kind) {
                UnitKind.VEHICLE.name -> VehicleUnit(
                    ...
                    unitId = u.unitId,
                    label = u.label,
                    name = NameParts(last = u.pedLast, first = u.pedFirst, middle = u.pedMiddle, suffix = u.pedSuffix),
                    dobIso = u.pedDobIso,
